...
def fase_geoestatica():
    ...
    np.savez_compressed(
        "result_talud_2021/geostatic_state_v2.npz",
...
def fase_falla():
    ...
    geostatic_file = "result_talud_2021/geostatic_state_v2.npz"
    ...
    xi, yi, xf, yf = 0, 0, 60, 0.5 # coordenas inicial y final del rect´angulo que define el dominio
    ...
    xi, yi, xf, yf = 0.0, 0.5, 60, 30.5	# coordenadas inicial y final del rect´angulo que define el dominio
    ...
    filename_npz = name1 + name + '_data_v2.npz'
    np.savez_compressed(
...
def graficar_resultados():
    ...
    from mpm_un.graphics import graphic_button, graphic_button2, graphic_button3, graphic_gif, graphic_video
    import matplotlib.pyplot as plt
    filename_npz = 'result_talud_2021/S14_G9_data_v2.npz'
    ...
    # --- Grafica 1: Puntos materiales - configuracion inicial por cuerpo ---
    fig0, ax0 = plt.subplots(figsize=(12, 4.5))
    plt.subplots_adjust(bottom=0.15)
    ax0.scatter(corX[:nmp_1, 0], corY[:nmp_1, 0], s=1, c='steelblue', label='Cuerpo 1 (estrato inferior)')
    ax0.scatter(corX[nmp_1:, 0], corY[nmp_1:, 0], s=1, c='tomato', label='Cuerpo 2 (talud)')
    ax0.set_xlabel('Coordenada $x$ (m)', fontsize=11)
    ax0.set_ylabel('Coordenada $y$ (m)', fontsize=11)
    ax0.set(xlim=(0, dimx), ylim=(0, dimy))
    ax0.set_aspect('equal')
    ax0.tick_params(labelsize=10)
    ax0.set_title('Configuracion inicial - Puntos Materiales', fontsize=12)
    ax0.legend(loc='upper right', fontsize=10, markerscale=6)
    ax0.grid(True)
    plt.tight_layout()
    plt.show()
...
if __name__ == "__main__":
    ...
    if not os.path.exists("result_talud_2021/geostatic_state_v2.npz"):
        print("Archivo 'geostatic_state_v2.npz' no encontrado. Iniciando fase_geoestatica...")
        fase_geoestatica()
    else:
        print("Archivo 'geostatic_state_v2.npz' encontrado. Saltando fase_geoestatica.")

    if not os.path.exists("result_talud_2021/S14_G9_data_v2.npz"):
        print("Archivo 'S14_G9_data_v2.npz' no encontrado. Iniciando fase_falla...")
        fase_falla()

    else:
        print("Archivo 'S14_G9_data_v2.npz' encontrado. Saltando fase_falla.")
